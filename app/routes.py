...
def login() -> Union[str, Response]:
    ...
    if request.method == "POST":
        ...
        remember = request.form.get("remember") == 'on'  # Improved handling of the "remember me" checkbox
        ...
        if user and check_password_hash(user.password, password):
            ...
            return redirect(url_for("routes.login", error="Invalid credentials"))  # Error handling

    # GET request to render the login page
    full_page: bool = request.args.get("full", "true").lower() == "true"
    error = request.args.get("error")  # Capture any error messages to display
    return render_template("login.html", full_page=full_page, error=error)
